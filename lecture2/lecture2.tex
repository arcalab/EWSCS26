...
\begin{frame}{A symmetric monoidal closed category}
        ...
        \[
               \xymatrix@C=50pt{
                       ...
                       \\
                       (B \multimap C) \otimes B \ar[r]_(0.55){\mathrm{app}} & C
               }
        \]
...
\begin{frame}{The different incarnations of swap}
        \[
                \sem{x  \vdash \lambda y. \> y \otimes x }
        \]

        \begin{align*}
                v \mapsto (u \mapsto (u,v)) & \hspace{1cm} \text{ in } \Set 
                \\[10pt]
                v \mapsto (u \mapsto u \otimes v) & \hspace{1cm} \text{ in } \CVec
                \\[10pt]
                a \leq (b + a) - b & \hspace{1cm} \text{ in } \CR
                \\[10pt]
                (v_i \mapsto (u_j \mapsto (u_j,v_i))_{j \in \Nats})_{i \in \Nats}
...
\begin{frame}{Exercise}
        ...
        In any smc category the two morphisms below are equal
        \[
                \xymatrix@C=40pt{
                        A \ar[r]^(0.3){\lambda(sw)} 
                        \ar@/_1.5pc/[rr]_{\lambda(\id)} 
                        & (B \multimap B \otimes A)
                        \ar[r]^{\lambda(sw \comp app) } &
                        (B \multimap A \otimes B)
                }
        \]

        Prove that such is true via $\lambda$-calculus and its eq. system

        Hint: arrow composition corresponds to substitution
\end{frame}
\end{document}
